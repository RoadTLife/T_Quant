# 克隆自聚宽文章：https://www.joinquant.com/post/72335
# 标题：只适合目前市场的打板策略
# 作者：zjj2026888

# 克隆自聚宽文章：https://www.joinquant.com/post/70935
# 标题：《三马+小市值+七星高照+白马》未来函数拟合分析和现实优化
# 作者：NappyQuanter
...
# 异常处理窗口期检查
    ...
    g.rsi_threshold = 85 #几乎无法触发（RSI>98非常罕见），降低至80~85，用于剔除超买严重的ETF。 
    ...
    for stock in stock_list:
        ...
        try:
            ...
            if score < max_tolerance:  
                ...
                log.info(f"[排雷剔除] ??? {stock}({stock_name}) 踩雷 {score} 项，超过当前时代容忍度({max_tolerance})，已精准拦截!")
...
def check_macd_divergence(context, market_index="399101.XSHE", end_days=0):
    ...
    if detect_divergence():
        g.dbl.append(1)
        log.warn(f"[顶背离] ?? 检测到{market_index}顶背离信号（价格新高但MACD走弱），清仓非涨停股票")
